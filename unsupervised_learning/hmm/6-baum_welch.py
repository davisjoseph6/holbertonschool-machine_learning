...
def forward(Observation, Emission, Transition, Initial):
    ...
    # Recursion step: Compute forward probabilities for each subsequent
    # observation
    for t in range(1, T):
        for j in range(N):
            F[j, t] = np.sum(
...
def backward(Observation, Emission, Transition):
    ...
    # Initialization step: Set backward probabilities for the last observation
    # to 1
    B[:, T-1] = 1

    # Recursion step: Compute backward probabilities for each previous
    # observation
...
def baum_welch(Observations, Transition, Emission, Initial, iterations=1000):
    ...
    """
    N, M = Emission.shape  # N = N. of hidden states, M = N. of observations
