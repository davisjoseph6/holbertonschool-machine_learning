...
class FileLogger(Callback):
    ...
    def on_episode_end(self, episode, logs):
        ...
        mean_metrics = np.nanmean(metrics, axis=0) if not np.isnan(metrics).all() else np.array([np.nan] * len(self.metrics_names))
    ...
    def save_data(self):
        ...
        sorted_data = {key: np.array([self.data[key][idx] for idx in sorted_indexes]).tolist() for key in self.data}
